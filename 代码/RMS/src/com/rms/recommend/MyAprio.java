...
public class MyAprio {
	 ...
	 * @throws UnsupportedEncodingException 
	 */
	private static void printConfItemset(List<List<String>> confItemset2, PrintWriter pw) throws UnsupportedEncodingException, FileNotFoundException {
		...
		for (int i = 0; i < confItemset2.size(); i++) {
			...
			pw.print(confItemset2.get(i).get(j) + " ");
			pw.print("-->");
			pw.print(confItemset2.get(i).get(j++));
			pw.print("   相对支持度：" + confItemset2.get(i).get(j++));
			pw.print("   自信度：" + confItemset2.get(i).get(j++) + "\n");
		}
	}
	...
	private static List<List<String>> getConfidencedItemset(
			...
			Map<Integer, Integer> dkCountMap2, Map<Integer, Integer> dCountMap2,Double parameter1) {
		for (int i = 0; i < lkItemset.size(); i++) {
			getConfItem(lkItemset.get(i), lItemset, dkCountMap2.get(i),
					dCountMap2, parameter1);
	...
	private static List<String> getConfItem(List<String> list,
			...
			Map<Integer, Integer> dCountMap2,Double MIN_CONF) {
	...
	private static List<List<String>> getSupportedItemset(
			List<List<String>> cItemset,Double MIN_SUPPORT) {
		// TODO Auto-generated method stub
		boolean end = true;
