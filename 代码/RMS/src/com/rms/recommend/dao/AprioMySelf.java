...
public class AprioMySelf {
	...
	public List<String> getApriori(Users u) throws ParseException{
		...
		//获取当前用户的搜索词汇
		List<String> search = session.createQuery("select distinct title from Search where searcher ="+u.getId()).list();
		System.out.println("search是否为空："+search.size());
		System.out.println("当前用户的搜索过的词汇："+search);
		/*展示数据
		//控制台输出记录
		...
		AssociationRulesMining(search); //挖掘关联规则
		return result;
		}
/**********************************************
     * ****************读取数据
 * @throws ParseException ********************/
	public List<List<String>> getRecord() throws ParseException {
		
		/*List<List<String>> record = new ArrayList<List<String>>();
		Session session = sf.getCurrentSession();
		//获取所有查询过的用户
		Query q= session.createQuery("select distinct seacher form Search");
		List<Integer> alluser =q.list();
		//获取
		...
		*/
		...
		Session session = sf.getCurrentSession();
		//获取所有搜索过的用户
		Query q= session.createQuery("select distinct searcher from Search");
		List<Users> alluser =q.list();
		//System.out.println("用户个数："+alluser.size());
		SimpleDateFormat simpleFormat = new SimpleDateFormat("yyyy-MM-dd hh:mm");
		//获取每个用户的搜索记录的频繁项集
		for(int i=0;i<alluser.size();i++) {
			//获取当前用户的所有搜索记录
			List<Search> searchs = session.createQuery("From Search s where s.searcher = "+alluser.get(i).getId()+"order by s.time asc").list();
			//遍历获取集合
			//System.out.println("当前用户："+alluser.get(i).getId()+",搜索记录条数："+searchs.size());
			boolean b=false;
			List<String> items = new ArrayList<String>();
			for(int j=1;j<searchs.size();j++) {
				if(j==1) {
					items.add(searchs.get(0).getTitle());//将第一个插入
					b=true;
				}
				b=false;
				//计算时间差
				//System.out.println("前一个的时间"+searchs.get(j).getTime().toString());
				//System.out.println("后一个的时间"+searchs.get(j-1).getTime().toString());
				 Date fromDate3 = simpleFormat.parse(searchs.get(j-1).getTime().toString());  
				    Date toDate3 = simpleFormat.parse(searchs.get(j).getTime().toString());  
				    long from3 = fromDate3.getTime();  
				    long to3 = toDate3.getTime();
				    //System.out.println("long类型的时间差结果："+(to3-from3));
				    int minutes = (int) ((to3 - from3) / (1000 * 60)); 
				    //System.out.println("各时间间隔："+minutes);
				    if(minutes<=30) {
				    	items.add(searchs.get(j).getTitle());
				    	b=true;
				    	//System.out.println("小于三十");
				    }
				    if(!b) {
						record.add(items);
		...
		//System.out.println("查看record的大小："+record.size());
    ...
    public static void AssociationRulesMining(List<String> search)//关联规则挖掘
    {
        for(int i=0;i<frequentItemset.size();i++)
        {
            ...
            if(tem.size()>1) {
                ...
                for (int j = 0; j < AllSubset.size(); j++) {
                    ...
                    double conf = isAssociationRules(s1, s2, tem,search);
                   // if (conf > 0)
    ...
    public  static  double isAssociationRules(List<String> s1,List<String> s2,List<String> tem,List<String> search)//判断是否为关联规则
    {
